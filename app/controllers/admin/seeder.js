...
export default Controller.extend({
  ...
  actions: {
    ...
    generateBooksAndAuthors(volume) {
      ...
      RSVP.all(booksWithAuthors).then(() => {
    ...
    deleteBooksAndAuthors() {
      ...
      this._destroyAll(authors).then(() => this._destroyAll(books)).then(() => {
  ...
  _saveRandomLibrary() {
    this.store.createRecord('library').randomize().save();
  },

  _destroyAll(records) {
    const recordsAreDestroying = records.map((item) => item.destroyRecord());
    return RSVP.all(recordsAreDestroying);
  },
  ...
  _generateSomeBooks(author) {
    ...
    for (let j = 0; j < bookCounter; j++) {
      ...
      const bookPromise = this.store.createRecord('book')
